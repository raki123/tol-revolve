...
MultiNNSpecies::MultiNNSpecies(int i,bool n) {
    id=i;
    age=1;
    ave_fitness=0.0;
    expected_offspring=0;
...
void MultiNNSpecies::remove_eliminated() {
    erase_if(organisms, [](MultiNNSpeciesOrganism *org) {return org->eliminate;});
}

void MultiNNSpecies::remove_generation(int gen) {
    erase_if(organisms, [gen](MultiNNSpeciesOrganism *org) {return org->generation == gen;});
}
...
bool MultiNNSpecies::print_to_file(std::ostream &outFile) {
    //Print a comment on the Species info
    outFile<<std::endl<<"/* MultiNNSpecies #"<<id<<" : (Size "<<organisms.size()<<") (AF "<<ave_fitness<<") (Age "<<age<<")  */"<<std::endl<<std::endl;
...
void MultiNNSpecies::adjust_fitness() {
    ...
    std::sort(organisms.begin(), organisms.end(),
              [] (MultiNNSpeciesOrganism *x, MultiNNSpeciesOrganism *y) {
...
static MultiNNSpeciesOrganism *get_random(rng_t &rng, MultiNNSpecies *thiz, const vector<MultiNNSpecies *> &sorted_species) {
...
void MultiNNSpecies::reproduce(int ioffspring,
...
bool NEAT::order_species(MultiNNSpecies *x, MultiNNSpecies *y) {
    return (((*((x->organisms).begin()))->eval.fitness) > ((*((y->organisms).begin()))->eval.fitness));
}

bool NEAT::order_new_species(MultiNNSpecies *x, MultiNNSpecies *y) {
